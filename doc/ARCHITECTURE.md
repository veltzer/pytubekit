...
Thin wrapper around the `youtube-dl` library:

- **`youtube_dl_download_urls()`** - Downloads videos/playlists using youtube-dl with `extract_flat` mode and `.netrc` authentication.
